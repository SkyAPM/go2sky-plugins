...
type UserFunc func(context.Context, *entschema.UserMutation) (entschema.Value, error)

// Mutate calls f(ctx, m).
func (f UserFunc) Mutate(ctx context.Context, m entschema.Mutation) (entschema.Value, error) {
	mv, ok := m.(*entschema.UserMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *entschema.UserMutation", m)
	}
	return f(ctx, mv)
}
