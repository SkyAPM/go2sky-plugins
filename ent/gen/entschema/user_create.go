...
func (uc *UserCreate) Save(ctx context.Context) (*User, error) {
	...
	if len(uc.hooks) == 0 {
		...
		if _, err1 := mut.Mutate(ctx, uc.mutation); err1 != nil {
			return nil, err1
		}
	}
	return node, err
...
func (uc *UserCreate) check() error {
	if _, ok := uc.mutation.Age(); !ok {
		return &ValidationError{Name: "age", err: errors.New(`entschema: missing required field "User.age"`)}
	}
	if _, ok := uc.mutation.Name(); !ok {
		return &ValidationError{Name: "name", err: errors.New(`entschema: missing required field "User.name"`)}
	}
	return nil
}
...
func (u *UserUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("entschema: missing options for UserCreate.OnConflict")
	}
...
// ID Exec executes the UPSERT query and returns the inserted/updated ID.
...
func (u *UserUpsertBulk) Exec(ctx context.Context) error {
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("entschema: OnConflict was set for builder %d. Set it on the UserCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("entschema: missing options for UserCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
